        ...
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <div className="flex items-center space-x-3 border-l border-slate-200 dark:border-slate-700 pl-4">
              ...
              {currentUser ? (
                <>
                  <div className="flex items-center space-x-2 bg-primary-50 dark:bg-primary-900/30 px-4 py-2 rounded-full border border-primary-200 dark:border-primary-800">
                    <User className="w-4 h-4 text-primary-600 dark:text-primary-400" />
                    <span className="text-sm font-bold text-primary-700 dark:text-primary-300 max-w-[120px] truncate">
                      {currentUser.name}
                    </span>
                  </div>
                  <button
                    onClick={handleLogout}
                    className="flex items-center space-x-2 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40 text-red-600 dark:text-red-400 px-4 py-2 rounded-full font-medium transition-colors border border-red-200 dark:border-red-800"
                  >
          ...
          >
            <div className="px-4 pt-2 pb-6 space-y-1">
              ...
              <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800 flex flex-col space-y-3">
                {currentUser ? (
                  <>
                    <div className="px-3 py-2 text-sm font-bold text-primary-600 dark:text-primary-400">
                      👋 Hello, {currentUser.name}!
                    </div>
                    <button
                      onClick={handleLogout}
                      className="w-full text-center px-4 py-3 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 font-medium border border-red-200 dark:border-red-800"
                    >
                      Logout
                    </button>
                  </>
